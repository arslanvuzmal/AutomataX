# Proactive SSL Expiration Prevention
...
Expired SSL certificates cause immediate customer-facing outages and security warnings, yet they are often tracked poorly in manual spreadsheets.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Proactive SSL Expiration Prevention**. 
...
![Workflow Architecture](../../assets/workflows/06-04/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `SSL Checker API`, `Jira`, `PagerDuty`, `Confluence`.
...
- **SSL Checker API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Confluence**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (SSL Checker API, Jira).
...
- `SSL Checker API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
- `Confluence API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Proactive SSL Expiration Prevention JSON](../../workflows/06-itsm-devops/06_04_proactive_ssl_expiration_prevention.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/06-itsm-devops/06_04_proactive_ssl_expiration_prevention.json)**
