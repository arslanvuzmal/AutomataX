# Proactive Feature Drop-Off Alerting
...
CSMs are often blindsided by churn because they lack visibility into when a customer stops using a key sticky feature in the product.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Proactive Feature Drop-Off Alerting**. 
...
![Workflow Architecture](../../assets/workflows/04-01/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Segment`, `Gainsight`, `Slack`.
...
- **Segment**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Gainsight**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Segment, Gainsight).
...
- `Segment API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Gainsight API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Proactive Feature Drop-Off Alerting JSON](../../workflows/04-customer-success/04_01_proactive_feature_drop_off_alerting.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/04-customer-success/04_01_proactive_feature_drop_off_alerting.json)**
