# Self-Healing Database Backup Verification
...
Backups are scheduled, but rarely tested. A corrupted backup is useless during a ransomware attack or data loss event.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Self-Healing Database Backup Verification**. 
...
![Workflow Architecture](../../assets/workflows/06-05/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `AWS RDS`, `Checksum Script`, `Alerting (Slack/Email)`.
...
- **AWS RDS**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Checksum Script**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Alerting (Slack/Email)**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (AWS RDS, Checksum Script).
...
- `AWS RDS API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Checksum Script API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Alerting (Slack/Email) API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Self-Healing Database Backup Verification JSON](../../workflows/06-itsm-devops/06_05_self_healing_database_backup_verification.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/06-itsm-devops/06_05_self_healing_database_backup_verification.json)**
