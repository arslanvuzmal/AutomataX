...
Onboarding a new vendor without a proper security review exposes the company to supply-chain cyber attacks. Doing it manually slows down procurement.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Third-Party Vendor Risk Assessment**. 
...
![Workflow Architecture](../../assets/workflows/08-04/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `ERP`, `MS Teams / Forms`, `Risk Scoring Logic`.
...
- **ERP**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **MS Teams / Forms**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Risk Scoring Logic**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (ERP, MS Teams / Forms).
...
- `ERP API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `MS Teams / Forms API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Risk Scoring Logic API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Third-Party Vendor Risk Assessment JSON](../../workflows/08-legal-compliance/08_04_third_party_vendor_risk_assessment.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/08-legal-compliance/08_04_third_party_vendor_risk_assessment.json)**
