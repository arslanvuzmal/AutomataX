# Crypto Treasury Real-Time P&L Tracking
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Crypto Treasury Real-Time P&L Tracking**. 
...
![Workflow Architecture](../../assets/workflows/01-07/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Etherscan API`, `Notion`, `PagerDuty`.
...
- **Etherscan API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Etherscan API, Notion).
...
- `Etherscan API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Crypto Treasury Real-Time P&L Tracking JSON](../../workflows/01-finops/01_07_crypto_treasury_real_time_p_l_tracking.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/01-finops/01_07_crypto_treasury_real_time_p_l_tracking.json)**
