# Executive CSAT/NPS Correlation
...
NPS scores exist in a silo. Executives need to know if the unhappy customers are your largest enterprise accounts or your lowest-tier users.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Executive CSAT/NPS Correlation**. 
...
![Workflow Architecture](../../assets/workflows/04-07/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Delighted`, `Salesforce`, `Notion`.
...
- **Delighted**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Delighted, Salesforce).
...
- `Delighted API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Executive CSAT/NPS Correlation JSON](../../workflows/04-customer-success/04_07_executive_csat_nps_correlation.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/04-customer-success/04_07_executive_csat_nps_correlation.json)**
