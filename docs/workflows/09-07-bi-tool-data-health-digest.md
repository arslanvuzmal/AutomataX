...
The data team has no idea which dashboards are actually being used by the business, leading to wasted compute costs refreshing abandoned reports.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **BI Tool "Data Health" Digest**. 
...
![Workflow Architecture](../../assets/workflows/09-07/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Tableau`, `Snowflake Metadata`, `Slack`.
...
- **Tableau**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Snowflake Metadata**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Tableau, Snowflake Metadata).
...
- `Tableau API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Snowflake Metadata API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [BI Tool "Data Health" Digest JSON](../../workflows/09-data-engineering/09_07_bi_tool_data_health_digest.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/09-data-engineering/09_07_bi_tool_data_health_digest.json)**
