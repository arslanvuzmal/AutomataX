...
High-value inbound leads get lost in round-robin queues or are assigned to the wrong reps, killing momentum and decreasing conversion rates.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Intelligent Lead Routing & Scoring**. 
...
![Workflow Architecture](../../assets/workflows/05-01/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `WordPress`, `Clearbit`, `Salesforce`.
...
- **Clearbit**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (WordPress, Clearbit).
...
- `Clearbit API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Intelligent Lead Routing & Scoring JSON](../../workflows/05-sales-crm/05_01_intelligent_lead_routing_scoring.json) and import into n8n canvas.
...
- **Fixture Input**: `fixtures/inputs/05-01.json`
- **Expected Result**: `fixtures/expected/05-01.json`
...
📥 **[Download n8n Workflow JSON](../../workflows/05-sales-crm/05_01_intelligent_lead_routing_scoring.json)**
