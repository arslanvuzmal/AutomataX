# Dynamic PII Masking for Dev Environments
...
Copying production databases to dev environments for testing often accidentally exposes sensitive customer PII to developers, causing massive compliance breaches.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Dynamic PII Masking for Dev Environments**. 
...
![Workflow Architecture](../../assets/workflows/09-05/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Database Cloning API`, `Masking Script`.
...
- **Database Cloning API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Masking Script**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Database Cloning API, Masking Script).
...
- `Database Cloning API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Masking Script API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Dynamic PII Masking for Dev Environments JSON](../../workflows/09-data-engineering/09_05_dynamic_pii_masking_for_dev_environments.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/09-data-engineering/09_05_dynamic_pii_masking_for_dev_environments.json)**
