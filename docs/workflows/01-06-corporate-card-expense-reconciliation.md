...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Corporate Card Expense Reconciliation**. 
...
![Workflow Architecture](../../assets/workflows/01-06/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Brex`, `SharePoint`, `AI Vision`, `QuickBooks Online`.
...
- **Brex**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **SharePoint**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **AI Vision**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **QuickBooks Online**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Brex, SharePoint).
...
- `Brex API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `SharePoint API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `AI Vision API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `QuickBooks Online API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Corporate Card Expense Reconciliation JSON](../../workflows/01-finops/01_06_corporate_card_expense_reconciliation.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/01-finops/01_06_corporate_card_expense_reconciliation.json)**
