# Proactive Employee Sentiment & Churn Risk Alerting
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Proactive Employee Sentiment & Churn Risk Alerting**. 
...
![Workflow Architecture](../../assets/workflows/02-05/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Culture Amp`, `Outlook`, `Slack/Teams`.
...
- **Culture Amp**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Outlook**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Slack/Teams**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Culture Amp, Outlook).
...
- `Culture Amp API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Outlook API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Slack/Teams API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Proactive Employee Sentiment & Churn Risk Alerting JSON](../../workflows/02-hr/02_05_proactive_employee_sentiment_churn_risk_alerting.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/02-hr/02_05_proactive_employee_sentiment_churn_risk_alerting.json)**
