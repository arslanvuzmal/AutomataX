# Headless Checkout Tax Liability Sync
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Headless Checkout Tax Liability Sync**. 
...
![Workflow Architecture](../../assets/workflows/01-10/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Avalara`, `Headless Checkout API`, `Workday`.
...
- **Avalara**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Headless Checkout API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Workday**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Avalara, Headless Checkout API).
...
- `Avalara API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Headless Checkout API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Workday API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Headless Checkout Tax Liability Sync JSON](../../workflows/01-finops/01_10_headless_checkout_tax_liability_sync.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/01-finops/01_10_headless_checkout_tax_liability_sync.json)**
