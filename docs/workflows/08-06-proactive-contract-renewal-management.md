...
Contracts auto-renew without renegotiation, or worse, expire without notice, leaving the business legally exposed or paying for unused software.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Proactive Contract Renewal Management**. 
...
![Workflow Architecture](../../assets/workflows/08-06/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Ironclad`, `Quote Generator`, `Email/Slack`.
...
- **Ironclad**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Quote Generator**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Ironclad, Quote Generator).
...
- `Ironclad API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Quote Generator API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Proactive Contract Renewal Management JSON](../../workflows/08-legal-compliance/08_06_proactive_contract_renewal_management.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/08-legal-compliance/08_06_proactive_contract_renewal_management.json)**
