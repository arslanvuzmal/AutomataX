# Automated Supplier Quality Scorecarding
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Automated Supplier Quality Scorecarding**. 
...
![Workflow Architecture](../../assets/workflows/03-03/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `WMS`, `QA Systems`, `ERP (e.g., SAP/Oracle)`.
...
- **WMS**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **QA Systems**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **ERP (e.g., SAP/Oracle)**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (WMS, QA Systems).
...
- `WMS API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `QA Systems API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `ERP (e.g., SAP/Oracle) API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Automated Supplier Quality Scorecarding JSON](../../workflows/03-supply-chain/03_03_automated_supplier_quality_scorecarding.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/03-supply-chain/03_03_automated_supplier_quality_scorecarding.json)**
