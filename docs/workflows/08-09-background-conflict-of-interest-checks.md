# Background Conflict of Interest Checks
...
Employees may accidentally have side-gigs or previous employment with a direct competitor or active vendor, creating severe conflicts of interest.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Background Conflict of Interest Checks**. 
...
![Workflow Architecture](../../assets/workflows/08-09/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `HRIS`, `CRM (Client Database)`, `Approval System`.
...
- **HRIS**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **CRM (Client Database)**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Approval System**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (HRIS, CRM (Client Database)).
...
- `HRIS API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `CRM (Client Database) API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Approval System API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Background Conflict of Interest Checks JSON](../../workflows/08-legal-compliance/08_09_background_conflict_of_interest_checks.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/08-legal-compliance/08_09_background_conflict_of_interest_checks.json)**
