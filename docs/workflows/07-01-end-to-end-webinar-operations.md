...
Hosting a webinar requires manually syncing registrants between Zoom, Marketo, and Salesforce, often resulting in missed follow-ups and lost leads.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **End-to-End Webinar Operations**. 
...
![Workflow Architecture](../../assets/workflows/07-01/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Zoom`, `Marketo`, `Salesforce`.
...
- **Zoom**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Marketo**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Zoom, Marketo).
...
- `Zoom API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Marketo API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [End-to-End Webinar Operations JSON](../../workflows/07-marketing/07_01_end_to_end_webinar_operations.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/07-marketing/07_01_end_to_end_webinar_operations.json)**
