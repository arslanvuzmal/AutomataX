...
Sales reps hate writing notes in the CRM, resulting in missing context for Account Managers post-sale and poor handoffs.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Call Intelligence Sync**. 
...
![Workflow Architecture](../../assets/workflows/05-06/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Gong`, `AI`, `Salesforce`.
...
- **Gong**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Gong, AI).
...
- `Gong API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Call Intelligence Sync JSON](../../workflows/05-sales-crm/05_06_call_intelligence_sync.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/05-sales-crm/05_06_call_intelligence_sync.json)**
