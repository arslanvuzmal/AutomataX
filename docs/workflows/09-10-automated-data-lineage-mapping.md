...
When a base table changes, data engineers have no way to know which downstream reports will break because documentation is completely siloed.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Automated Data Lineage Mapping**. 
...
![Workflow Architecture](../../assets/workflows/09-10/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Databricks (Query History)`, `SQL Parser`, `Data Catalog`.
...
- **Databricks (Query History)**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **SQL Parser**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Data Catalog**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Databricks (Query History), SQL Parser).
...
- `Databricks (Query History) API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `SQL Parser API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Data Catalog API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Automated Data Lineage Mapping JSON](../../workflows/09-data-engineering/09_10_automated_data_lineage_mapping.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/09-data-engineering/09_10_automated_data_lineage_mapping.json)**
