# Cross-Departmental SLA Enforcer
...
Sales blames CS for slow onboarding; CS blames Sales for poor handoffs. Without tracked SLAs between departments, the customer experience suffers.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Cross-Departmental SLA Enforcer**. 
...
![Workflow Architecture](../../assets/workflows/10-05/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Salesforce`, `Ops Dashboard`.
...
- **Ops Dashboard**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Salesforce, Ops Dashboard).
...
- `Ops Dashboard API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Cross-Departmental SLA Enforcer JSON](../../workflows/10-executive/10_05_cross_departmental_sla_enforcer.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/10-executive/10_05_cross_departmental_sla_enforcer.json)**
