...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Tier-1 Supplier Risk Trigger**. 
...
![Workflow Architecture](../../assets/workflows/03-09/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Resilinc API`, `ERP`.
...
- **Resilinc API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Resilinc API, ERP).
...
- `Resilinc API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Tier-1 Supplier Risk Trigger JSON](../../workflows/03-supply-chain/03_09_tier_1_supplier_risk_trigger.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/03-supply-chain/03_09_tier_1_supplier_risk_trigger.json)**
