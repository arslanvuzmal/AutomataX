# Instant VIP Brand Mention Triage
...
By the time a PR team notices a high-profile influencer complaining about the brand on Twitter, the tweet has already gone viral.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Instant VIP Brand Mention Triage**. 
...
![Workflow Architecture](../../assets/workflows/07-05/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Mention API`, `Slack`, `AI`.
...
- **Mention API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Mention API, Slack).
...
- `Mention API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Instant VIP Brand Mention Triage JSON](../../workflows/07-marketing/07_05_instant_vip_brand_mention_triage.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/07-marketing/07_05_instant_vip_brand_mention_triage.json)**
