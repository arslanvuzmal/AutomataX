...
By the time a support ticket breaches its SLA, the customer is already angry and contract penalties may apply. You need to act *before* the breach.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Pre-Breach SLA Escalation**. 
...
![Workflow Architecture](../../assets/workflows/04-08/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Freshdesk`, `SMS API (Twilio)`.
...
- **Freshdesk**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **SMS API (Twilio)**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Freshdesk, SMS API (Twilio)).
...
- `Freshdesk API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `SMS API (Twilio) API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Pre-Breach SLA Escalation JSON](../../workflows/04-customer-success/04_08_pre_breach_sla_escalation.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/04-customer-success/04_08_pre_breach_sla_escalation.json)**
