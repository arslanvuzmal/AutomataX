...
When the product team releases a new feature, support agents are often left using outdated macros, providing incorrect information to users.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Macro-Update Propagation**. 
...
![Workflow Architecture](../../assets/workflows/04-10/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Productboard`, `Zendesk`, `Slack`.
...
- **Productboard**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Zendesk**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Productboard, Zendesk).
...
- `Productboard API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Zendesk API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Macro-Update Propagation JSON](../../workflows/04-customer-success/04_10_macro_update_propagation.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/04-customer-success/04_10_macro_update_propagation.json)**
