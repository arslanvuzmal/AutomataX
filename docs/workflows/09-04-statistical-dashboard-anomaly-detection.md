# Statistical Dashboard Anomaly Detection
...
Business owners don't look at dashboards every day. When a key metric crashes, it goes unnoticed unless a data analyst spots it manually.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Statistical Dashboard Anomaly Detection**. 
...
![Workflow Architecture](../../assets/workflows/09-04/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Looker`, `Statistical Scripting`, `Email/Slack`.
...
- **Looker**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Statistical Scripting**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Email/Slack**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Looker, Statistical Scripting).
...
- `Looker API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Statistical Scripting API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Email/Slack API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Statistical Dashboard Anomaly Detection JSON](../../workflows/09-data-engineering/09_04_statistical_dashboard_anomaly_detection.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/09-data-engineering/09_04_statistical_dashboard_anomaly_detection.json)**
