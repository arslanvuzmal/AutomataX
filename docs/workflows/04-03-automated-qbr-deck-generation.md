...
![Category](https://img.shields.io/badge/Category-Customer%20Success-58A6FF.svg)
...
CSMs spend hours every quarter manually compiling usage graphs and support metrics to build Quarterly Business Review (QBR) slide decks.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Automated QBR Deck Generation**. 
...
![Workflow Architecture](../../assets/workflows/04-03/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Mixpanel`, `Intercom`, `Google Slides API`, `Email`.
...
- **Mixpanel**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Intercom**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Google Slides API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Email**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Mixpanel, Intercom).
...
- `Mixpanel API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Intercom API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Google Slides API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Email API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Automated QBR Deck Generation JSON](../../workflows/04-customer-success/04_03_automated_qbr_deck_generation.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/04-customer-success/04_03_automated_qbr_deck_generation.json)**
