# Fraud-Resistant Vendor Payment Gateway
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Fraud-Resistant Vendor Payment Gateway**. 
...
![Workflow Architecture](../../assets/workflows/01-08/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Coupa`, `Fraud Database`, `SFTP / Banking Portal`.
...
- **Fraud Database**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **SFTP / Banking Portal**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Coupa, Fraud Database).
...
- `Fraud Database API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `SFTP / Banking Portal API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Fraud-Resistant Vendor Payment Gateway JSON](../../workflows/01-finops/01_08_fraud_resistant_vendor_payment_gateway.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/01-finops/01_08_fraud_resistant_vendor_payment_gateway.json)**
