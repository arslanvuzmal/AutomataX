# Zero-Touch Board Deck Assembly
...
The CEO and Chief of Staff spend a full week every quarter hunting down metrics across different departments to build the Board of Directors slide deck.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Zero-Touch Board Deck Assembly**. 
...
![Workflow Architecture](../../assets/workflows/10-01/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `NetSuite`, `ChartMogul`, `Jira`, `Google Slides API`.
...
- **ChartMogul**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (NetSuite, ChartMogul).
...
- `ChartMogul API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Zero-Touch Board Deck Assembly JSON](../../workflows/10-executive/10_01_zero_touch_board_deck_assembly.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/10-executive/10_01_zero_touch_board_deck_assembly.json)**
