# Automated Non-Prod Cloud Cost Optimization
...
Developers leave expensive dev/test environments running 24/7 over the weekend, driving up AWS/Azure bills unnecessarily.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Automated Non-Prod Cloud Cost Optimization**. 
...
![Workflow Architecture](../../assets/workflows/06-07/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `AWS Compute Optimizer`, `AWS Lambda`, `Slack`.
...
- **AWS Compute Optimizer**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **AWS Lambda**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (AWS Compute Optimizer, AWS Lambda).
...
- `AWS Compute Optimizer API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `AWS Lambda API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Automated Non-Prod Cloud Cost Optimization JSON](../../workflows/06-itsm-devops/06_07_automated_non_prod_cloud_cost_optimization.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/06-itsm-devops/06_07_automated_non_prod_cloud_cost_optimization.json)**
