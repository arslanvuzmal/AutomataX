...
IT helpdesks get bogged down manually granting software licenses based on ServiceNow tickets, slowing down employee productivity and risking over-provisioning.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Automated Role-Based Access Provisioning**. 
...
![Workflow Architecture](../../assets/workflows/06-02/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `ServiceNow`, `Okta`, `Software APIs`.
...
- **ServiceNow**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Okta**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Software APIs**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (ServiceNow, Okta).
...
- `ServiceNow API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Okta API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Software APIs API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Automated Role-Based Access Provisioning JSON](../../workflows/06-itsm-devops/06_02_automated_role_based_access_provisioning.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/06-itsm-devops/06_02_automated_role_based_access_provisioning.json)**
