# Automated Budget vs. Actuals Cloud Cost Monitoring
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Automated Budget vs. Actuals Cloud Cost Monitoring**. 
...
![Workflow Architecture](../../assets/workflows/01-02/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `AWS Cost Explorer`, `NetSuite`, `Jira`, `Slack`.
...
- **AWS Cost Explorer**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (AWS Cost Explorer, NetSuite).
...
- `AWS Cost Explorer API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Automated Budget vs. Actuals Cloud Cost Monitoring JSON](../../workflows/01-finops/01_02_automated_budget_vs_actuals_cloud_cost_monitoring.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/01-finops/01_02_automated_budget_vs_actuals_cloud_cost_monitoring.json)**
