# Self-Healing ETL Pipeline Monitor
...
When an Airbyte or Fivetran sync fails silently, data engineers only find out days later when executives complain about stale dashboards.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Self-Healing ETL Pipeline Monitor**. 
...
![Workflow Architecture](../../assets/workflows/09-01/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Fivetran/Airbyte Webhooks`, `PagerDuty`.
...
- **Fivetran/Airbyte Webhooks**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Fivetran/Airbyte Webhooks, PagerDuty).
...
- `Fivetran/Airbyte Webhooks API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Self-Healing ETL Pipeline Monitor JSON](../../workflows/09-data-engineering/09_01_self_healing_etl_pipeline_monitor.json) and import into n8n canvas.
...
- **Fixture Input**: `fixtures/inputs/09-01.json`
- **Expected Result**: `fixtures/expected/09-01.json`
...
📥 **[Download n8n Workflow JSON](../../workflows/09-data-engineering/09_01_self_healing_etl_pipeline_monitor.json)**
