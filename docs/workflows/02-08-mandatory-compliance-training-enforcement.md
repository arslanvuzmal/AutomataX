# Mandatory Compliance Training Enforcement
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Mandatory Compliance Training Enforcement**. 
...
![Workflow Architecture](../../assets/workflows/02-08/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Docebo`, `Slack`, `HRIS (e.g., BambooHR)`.
...
- **Docebo**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Slack**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **HRIS (e.g., BambooHR)**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Docebo, Slack).
...
- `Docebo API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Slack API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `HRIS (e.g., BambooHR) API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Mandatory Compliance Training Enforcement JSON](../../workflows/02-hr/02_08_mandatory_compliance_training_enforcement.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/02-hr/02_08_mandatory_compliance_training_enforcement.json)**
