# Dynamic Subscription Dunning & Retention
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Dynamic Subscription Dunning & Retention**. 
...
![Workflow Architecture](../../assets/workflows/01-04/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Stripe`, `Intercom`, `Salesforce`, `SMS/Email API`.
...
- **SMS/Email API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Stripe, Intercom).
...
- `SMS/Email API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Dynamic Subscription Dunning & Retention JSON](../../workflows/01-finops/01_04_dynamic_subscription_dunning_retention.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/01-finops/01_04_dynamic_subscription_dunning_retention.json)**
