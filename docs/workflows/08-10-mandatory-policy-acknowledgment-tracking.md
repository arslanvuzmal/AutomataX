# Mandatory Policy Acknowledgment Tracking
...
When an employee handbook is updated, proving that all 1,000 employees actually read and signed the new policy is a logistical nightmare.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Mandatory Policy Acknowledgment Tracking**. 
...
![Workflow Architecture](../../assets/workflows/08-10/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Gusto`, `Slack/Email`, `Tracking Database`.
...
- **Slack/Email**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Tracking Database**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Gusto, Slack/Email).
...
- `Slack/Email API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Tracking Database API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Mandatory Policy Acknowledgment Tracking JSON](../../workflows/08-legal-compliance/08_10_mandatory_policy_acknowledgment_tracking.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/08-legal-compliance/08_10_mandatory_policy_acknowledgment_tracking.json)**
