# Automated Visa & Relocation Compliance
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Automated Visa & Relocation Compliance**. 
...
![Workflow Architecture](../../assets/workflows/02-06/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `BambooHR`, `PDFMonkey`, `Asana`.
...
- **BambooHR**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **PDFMonkey**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Asana**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (BambooHR, PDFMonkey).
...
- `BambooHR API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `PDFMonkey API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Asana API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Automated Visa & Relocation Compliance JSON](../../workflows/02-hr/02_06_automated_visa_relocation_compliance.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/02-hr/02_06_automated_visa_relocation_compliance.json)**
