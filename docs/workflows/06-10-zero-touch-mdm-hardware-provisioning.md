# Zero-Touch MDM Hardware Provisioning
...
Setting up a new laptop manually with corporate software takes hours of IT time per employee, delaying their Day 1 productivity.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Zero-Touch MDM Hardware Provisioning**. 
...
![Workflow Architecture](../../assets/workflows/06-10/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `SnipeIT`, `Jamf`, `Email`.
...
- **Jamf**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (SnipeIT, Jamf).
...
- `Jamf API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Zero-Touch MDM Hardware Provisioning JSON](../../workflows/06-itsm-devops/06_10_zero_touch_mdm_hardware_provisioning.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/06-itsm-devops/06_10_zero_touch_mdm_hardware_provisioning.json)**
