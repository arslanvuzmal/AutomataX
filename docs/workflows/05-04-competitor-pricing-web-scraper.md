# Competitor Pricing Web Scraper
...
Competitors change pricing quietly. If sales enablement doesn't know, reps get blindsided on calls and lose deals to cheaper alternatives.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Competitor Pricing Web Scraper**. 
...
![Workflow Architecture](../../assets/workflows/05-04/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Web Scraper`, `Highspot`, `Slack`.
...
- **Web Scraper**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Highspot**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Web Scraper, Highspot).
...
- `Web Scraper API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Highspot API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Competitor Pricing Web Scraper JSON](../../workflows/05-sales-crm/05_04_competitor_pricing_web_scraper.json) and import into n8n canvas.
...
- **Fixture Input**: `fixtures/inputs/05-04.json`
- **Expected Result**: `fixtures/expected/05-04.json`
...
📥 **[Download n8n Workflow JSON](../../workflows/05-sales-crm/05_04_competitor_pricing_web_scraper.json)**
