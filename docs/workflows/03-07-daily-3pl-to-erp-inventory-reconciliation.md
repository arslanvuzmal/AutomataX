# Daily 3PL to ERP Inventory Reconciliation
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Daily 3PL to ERP Inventory Reconciliation**. 
...
![Workflow Architecture](../../assets/workflows/03-07/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `ShipHero`, `Microsoft Dynamics`.
...
- **ShipHero**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Microsoft Dynamics**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (ShipHero, Microsoft Dynamics).
...
- `ShipHero API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Microsoft Dynamics API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Daily 3PL to ERP Inventory Reconciliation JSON](../../workflows/03-supply-chain/03_07_daily_3pl_to_erp_inventory_reconciliation.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/03-supply-chain/03_07_daily_3pl_to_erp_inventory_reconciliation.json)**
