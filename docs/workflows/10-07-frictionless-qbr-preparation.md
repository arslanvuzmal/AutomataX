...
Quarterly Business Reviews for the executive team become messy because every department brings data formatted differently, making it hard to compare apples to apples.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Frictionless QBR Preparation**. 
...
![Workflow Architecture](../../assets/workflows/10-07/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Departmental Data Sources`, `Notion`.
...
- **Departmental Data Sources**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Departmental Data Sources, Notion).
...
- `Departmental Data Sources API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Frictionless QBR Preparation JSON](../../workflows/10-executive/10_07_frictionless_qbr_preparation.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/10-executive/10_07_frictionless_qbr_preparation.json)**
