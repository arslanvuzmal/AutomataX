# Intelligent API Rate Limit Handler
...
Simple ETL scripts often crash because they hit rate limits on external APIs (like Shopify or Zendesk), requiring manual engineering intervention to restart.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Intelligent API Rate Limit Handler**. 
...
![Workflow Architecture](../../assets/workflows/09-09/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `External APIs`, `Wait Nodes (n8n)`.
...
- **External APIs**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Wait Nodes (n8n)**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (External APIs, Wait Nodes (n8n)).
...
- `External APIs API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Wait Nodes (n8n) API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Intelligent API Rate Limit Handler JSON](../../workflows/09-data-engineering/09_09_intelligent_api_rate_limit_handler.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/09-data-engineering/09_09_intelligent_api_rate_limit_handler.json)**
