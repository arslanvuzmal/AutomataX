...
The wider company (sales, product, leadership) rarely logs into Customer Success software, creating a blind spot for account risks and expansion opportunities.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Account Health Slack Digest**. 
...
![Workflow Architecture](../../assets/workflows/04-09/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Totango`, `Slack`.
...
- **Totango**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Totango, Slack).
...
- `Totango API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Account Health Slack Digest JSON](../../workflows/04-customer-success/04_09_account_health_slack_digest.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/04-customer-success/04_09_account_health_slack_digest.json)**
