# Event Badge Scanner to CRM Sync
...
Sales reps collect hundreds of business cards at trade shows, but rarely enter them into the CRM promptly, killing post-event momentum.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Event Badge Scanner to CRM Sync**. 
...
![Workflow Architecture](../../assets/workflows/07-08/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `OCR Scanner App`, `HubSpot`, `Email Sequence`.
...
- **OCR Scanner App**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **HubSpot**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Email Sequence**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (OCR Scanner App, HubSpot).
...
- `OCR Scanner App API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `HubSpot API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Email Sequence API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Event Badge Scanner to CRM Sync JSON](../../workflows/07-marketing/07_08_event_badge_scanner_to_crm_sync.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/07-marketing/07_08_event_badge_scanner_to_crm_sync.json)**
