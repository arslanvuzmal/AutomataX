# Automated M&A Virtual Data Room Setup
...
During a merger or acquisition, standing up a secure data room and granting granular access to external lawyers and auditors is a massive administrative headache.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Automated M&A Virtual Data Room Setup**. 
...
![Workflow Architecture](../../assets/workflows/10-04/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `VDR API (e.g., Firmroom)`, `Identity Provider`.
...
- **VDR API (e.g., Firmroom)**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Identity Provider**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (VDR API (e.g., Firmroom), Identity Provider).
...
- `VDR API (e.g., Firmroom) API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Identity Provider API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Automated M&A Virtual Data Room Setup JSON](../../workflows/10-executive/10_04_automated_m_a_virtual_data_room_setup.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/10-executive/10_04_automated_m_a_virtual_data_room_setup.json)**
