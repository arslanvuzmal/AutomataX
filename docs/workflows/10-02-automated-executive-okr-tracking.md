# Automated Executive OKR Tracking
...
Department heads forget to update their OKRs. When leadership meets, half the data is stale, making strategic decision-making impossible.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Automated Executive OKR Tracking**. 
...
![Workflow Architecture](../../assets/workflows/10-02/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Notion`, `Email`, `Reporting Logic`.
...
- **Notion**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Email**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Reporting Logic**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Notion, Email).
...
- `Notion API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Email API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Reporting Logic API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Automated Executive OKR Tracking JSON](../../workflows/10-executive/10_02_automated_executive_okr_tracking.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/10-executive/10_02_automated_executive_okr_tracking.json)**
