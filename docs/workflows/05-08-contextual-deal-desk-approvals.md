# Contextual Deal Desk Approvals
...
Discount approvals are requested in chaotic Slack threads. Leadership lacks visibility into how a 15% discount actually impacts the margin of the deal.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Contextual Deal Desk Approvals**. 
...
![Workflow Architecture](../../assets/workflows/05-08/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Salesforce`, `Slack/Teams`, `ERP (Margin Data)`.
...
- **ERP (Margin Data)**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Salesforce, Slack/Teams).
...
- `ERP (Margin Data) API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Contextual Deal Desk Approvals JSON](../../workflows/05-sales-crm/05_08_contextual_deal_desk_approvals.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/05-sales-crm/05_08_contextual_deal_desk_approvals.json)**
