# The "CEO Morning Briefing" Engine

![Maturity: Demo Verified](https://img.shields.io/badge/Maturity-Demo%20Verified-green.svg)
![Category](https://img.shields.io/badge/Category-Executive%20Operations-58A6FF.svg)
...
Executives are overwhelmed by notifications across 10 different apps. They need a single, concise snapshot of the business's health before they start their day.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **The "CEO Morning Briefing" Engine**. 
...
![Workflow Architecture](../../assets/workflows/10-03/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `News API`, `Slack`, `BI Tool (e.g., Looker)`, `Email/SMS`.
...
- **News API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Slack**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **BI Tool (e.g., Looker)**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Email/SMS**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (News API, Slack).
...
- `News API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Slack API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `BI Tool (e.g., Looker) API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Email/SMS API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [The "CEO Morning Briefing" Engine JSON](../../workflows/10-executive/10_03_the_ceo_morning_briefing_engine.json) and import into n8n canvas.
...
- **Fixture Input**: `fixtures/inputs/10-03.json`
- **Expected Result**: `fixtures/expected/10-03.json`
...
📥 **[Download n8n Workflow JSON](../../workflows/10-executive/10_03_the_ceo_morning_briefing_engine.json)**
