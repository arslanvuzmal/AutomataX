# Secure Slackbot Password Resets
...
"I forgot my password" is the #1 IT support ticket, consuming massive amounts of Level 1 helpdesk time.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Secure Slackbot Password Resets**. 
...
![Workflow Architecture](../../assets/workflows/06-08/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Slackbot`, `Okta MFA`, `Active Directory`, `Email`.
...
- **Slackbot**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Okta MFA**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Active Directory**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (Slackbot, Okta MFA).
...
- `Slackbot API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Okta MFA API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Active Directory API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Secure Slackbot Password Resets JSON](../../workflows/06-itsm-devops/06_08_secure_slackbot_password_resets.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/06-itsm-devops/06_08_secure_slackbot_password_resets.json)**
