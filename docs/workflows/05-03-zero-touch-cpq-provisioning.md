...
Generating quotes manually introduces pricing errors, rogue discounting, and delays in getting the contract to the prospect while they are hot.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Zero-Touch CPQ & Provisioning**. 
...
![Workflow Architecture](../../assets/workflows/05-03/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Salesforce`, `PandaDoc`, `Provisioning API`.
...
- **Provisioning API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Salesforce, PandaDoc).
...
- `Provisioning API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Zero-Touch CPQ & Provisioning JSON](../../workflows/05-sales-crm/05_03_zero_touch_cpq_provisioning.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/05-sales-crm/05_03_zero_touch_cpq_provisioning.json)**
