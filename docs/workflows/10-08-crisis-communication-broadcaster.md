# Crisis Communication Broadcaster
...
During a PR crisis or severe outage, coordinating the message across the status page, customer emails, and internal Slack takes too long, leading to confused customers.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Crisis Communication Broadcaster**. 
...
![Workflow Architecture](../../assets/workflows/10-08/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `StatusPage`, `Email Marketing API`, `AI`, `Slack`.
...
- **StatusPage**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Email Marketing API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
...
- **Primary Action**: Records persisted or state updated across target platforms (StatusPage, Email Marketing API).
...
- `StatusPage API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Email Marketing API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
...
1. **Import Workflow**: Download [Crisis Communication Broadcaster JSON](../../workflows/10-executive/10_08_crisis_communication_broadcaster.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/10-executive/10_08_crisis_communication_broadcaster.json)**
