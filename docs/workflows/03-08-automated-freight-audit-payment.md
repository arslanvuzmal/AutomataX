...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Automated Freight Audit & Payment**. 
...
![Workflow Architecture](../../assets/workflows/03-08/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Email`, `PDF Parser`, `Database`.
...
- **PDF Parser**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Database**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Email, PDF Parser).
...
- `PDF Parser API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Database API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Automated Freight Audit & Payment JSON](../../workflows/03-supply-chain/03_08_automated_freight_audit_payment.json) and import into n8n canvas.
...
📥 **[Download n8n Workflow JSON](../../workflows/03-supply-chain/03_08_automated_freight_audit_payment.json)**
