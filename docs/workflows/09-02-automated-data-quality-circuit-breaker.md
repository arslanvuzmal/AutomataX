# Automated Data Quality Circuit Breaker
...
Bad data ingested from upstream systems pollutes downstream BI reports. By the time it's noticed, the data warehouse is already corrupted.
...
This AutomataX workflow orchestrates an end-to-end automated pipeline for **Automated Data Quality Circuit Breaker**. 
...
![Workflow Architecture](../../assets/workflows/09-02/architecture.svg)
...
3. **Integration Processing**: Interacts with core services: `Great Expectations`, `dbt`, `Snowflake`, `Alerting API`.
...
- **Great Expectations**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **dbt**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Snowflake**: Primary operational service responsible for data retrieval, state mutation, or record persistence.
- **Alerting API**: Primary operational service responsible for data retrieval, state mutation, or record persistence.

---

## Outputs

- **Primary Action**: Records persisted or state updated across target platforms (Great Expectations, dbt).
...
- `Great Expectations API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `dbt API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Snowflake API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.
- `Alerting API API Credentials`: Scoped access token or OAuth2 credential in n8n Credential Manager.

---

## Setup

1. **Import Workflow**: Download [Automated Data Quality Circuit Breaker JSON](../../workflows/09-data-engineering/09_02_automated_data_quality_circuit_breaker.json) and import into n8n canvas.
...
- **Fixture Input**: `fixtures/inputs/09-02.json`
- **Expected Result**: `fixtures/expected/09-02.json`
...
📥 **[Download n8n Workflow JSON](../../workflows/09-data-engineering/09_02_automated_data_quality_circuit_breaker.json)**
